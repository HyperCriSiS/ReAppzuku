...
| A03 | Privileged shell / Shizuku UserService | DECIDED | Readiness/death/rebind state is instrumented, queued service actions wait for a ready backend (`69750c7`, gate `34065892554`), and run `33997372602` proves real official-Shizuku UserService execution plus daemon-death/rebind recovery. Android 17/API 37 run `34316502181` additionally proves the real manager grant flow reaches `SHIZUKU_READY` and executes representative typed mutations through the real UserService; root-specific execution remains open. |
| A04 | Running-app discovery / app-state collection | DECIDED | ActivityManager process/service parsing is isolated and JVM fixture-tested across AOSP/OEM-style forms; real official-Shizuku API-36 `ProcessRecord` parsing is proven (`API36_PROCESS_RECORD_PARSED` in `34153570390`). Android 17/API 37 run `35646519446` now additionally proves the production parser against a real system-generated `ServiceRecord` carrying the current AOSP `c:<recentCallingPackage>` suffix; OEM/physical runtime-format diversity remains open. |
| A05 | Manual restrictions / freeze / force-stop / app ops | DECIDED | High-impact mutating operations use typed/validated `PrivilegedShell`; API-36 run `34005682619` and Android 17/API 37 run `34316502181` prove representative real-Shizuku command families and rollback behavior on disposable targets. The API-37 run covers AppOps, standby bucket, DeviceIdle whitelist, suspend/unsuspend, disable-user/enable, force-stop and explicit component broadcast with post-test rollback checks. Root-specific execution and deliberately destructive families remain separate evidence. |
...
| A10 | Accessibility / app-launch tracking | DECIDED | Service configuration and unnecessary view-tree scope were corrected; `AppLaunchTriggerPolicy` JVM-tests exact target eligibility/5-second duplicate suppression (`34153818522`), and `AccessibilityServicePolicyTest` locks the minimal `TYPE_WINDOW_STATE_CHANGED`-only service contract (`34156391150`). External API-37 run `34302578767` additionally proves Android 17 ECM-approved service enable/binding, a live ReAppzuku process and a real Settings window-state event reaching production code via `smart_last_foreground_com.android.settings`, while AutoKill and the app-launch mutation trigger remain disabled. Physical/OEM Accessibility diversity remains release evidence. |
...
| A16 | Exported surfaces: shortcuts / tiles / receivers / widget | DECIDED | Shortcut confused-deputy routing has an explicit principal policy (`f3145bb0`, `34154664082`) and manifest/documentation parity plus platform permissions are regression-tested (`34138775145`). The former one-shot external attacker check is now a permanent API-37 regression: `securityProbe` is a no-permission APK under a UID distinct from ReAppzuku; branch run `34668594681`, final branch run `34670073539`, merged-main run `34670295954`, and post-HiddenApi main run `34670916924` proved no Binder exposure from Accessibility/Quick Tiles and denial of explicit foreign `BOOT_COMPLETED`. OEM/launcher/widget-host diversity remains release evidence. |
...
| A18 | Build / CI / dependencies / supply chain | DECIDED | Source-authoritative least-privilege CI, immutable Action SHA pins, zero-error lint, dependency locking/verification and stable tag/source/APK version binding are enforced. Gradle is 9.7.1 with the binary distribution SHA-256 pinned; pinned Java/Kotlin CodeQL remains active. The signed-release task graph is explicitly scoped to `:app:` so the permanent `securityProbe` test module cannot enter release builds. As of 2026-09-21 GitHub reports zero open Dependabot security alerts and zero open secret-scanning alerts; CodeQL has one explicitly accepted medium warning (#9, `java/android/missing-certificate-pinning`) on the credential-free public GitHub release-metadata request. Root-shell fallback policy is regression-locked in `fd1f533b8c5a8ea51a3ecbc413b2fe17bf1f0910`. Physical release signing/rollback evidence remains open, with preflight `35642800123` proving fail-closed behavior while stable signing secrets are absent. |
...
| A10 Accessibility | D | R | R | D | D | **R** | - | - | **P** | D | R | D | **P** | **R** | R | R | - | **R** |
...
| A16 Exported entrypoints | D | R | R | R | R | D | D | - | D | D | R | R | **P** | R | - | D | R | **P** |
...
- **CM-P1-09:** accessibility service settings path/scope are corrected; `AccessibilityServicePolicyTest` locks the minimal window-state-only contract and normal run `34156391150` passed. External Android 17/API 37 run `34302578767` then modeled explicit ECM approval, observed the production service enabled/binding with a live app process, launched real System Settings and proved the production foreground event by `smart_last_foreground_com.android.settings`; both AutoKill and the app-launch mutation trigger remained disabled. Physical/OEM diversity remains separate release evidence.
...
- commit `ad672fc661f27345e075f325d180ddcc1fca1306`: real Android 17/API-37 ActivityManager `ServiceRecord` parsing is proven with a debug-only non-exported bound target service and `UiAutomation` system dump; diagnostic run `35645713842` captured the current AOSP `c:<recentCallingPackage>` format, final standard `35645966192`, CodeQL `35645972117`, targeted API-37 `35646059393` and full API-37 `35646519446` passed, with the full lane reporting `OK (42 tests)`, separate-UID abuse-probe PASS and launcher smoke;
- commit `fd1f533b8c5a8ea51a3ecbc413b2fe17bf1f0910`: the audited absolute root-shell fallback contract is a permanent JVM regression guard covering all eight supported `su` candidates in order and rejecting bare PATH-based `exec("su")`; standard `35643580520` and PR CodeQL `35643571945` passed. Real KernelSU/Magisk execution remains device evidence;
...
- workflow run `34671036949`: final `main` normal validation after permanent API-37 security-probe integration and HiddenApiBypass removal passed unit tests, lint, AndroidTest compilation, Room schema verification and debug APK build/upload;
- workflow run `34670916924`: final `main` Android 17/API 37 gate after HiddenApiBypass removal built and installed app + androidTest + separate no-permission `securityProbe`, passed instrumentation, external foreign-UID abuse probe and launcher crash smoke;
- workflow run `34670295954`: first merged-main API-37 validation of the permanent external `securityProbe` passed build/install, instrumentation, foreign Binder/broadcast abuse checks and launcher smoke;
- workflow run `34670291980`: first merged-main normal validation of the permanent external security-probe product line passed all normal source/build gates;
- workflow run `34668611855`: `main` normal validation passed after the Gradle wrapper moved to 9.7.1 with distribution checksum pinning;
- workflow runs `34672629853` / `34672626846`: pinned Java/Kotlin CodeQL entered the permanent assurance line with normal + CodeQL validation passing; repaired-head validation after the actionable findings passed normal `34673771828`, CodeQL `34673776649` / PR `34673964881`, and API-37 `34673781807`;
- repository security state checked 2026-09-21: zero open Dependabot alerts and zero open secret-scanning alerts. CodeQL has one intentionally accepted medium warning (#9, `java/android/missing-certificate-pinning`) on the credential-free public `api.github.com` release-metadata request; standard platform TLS validation is retained because fixed pins against GitHub's externally managed/rotating certificates would create update-availability fragility;
...
- workflow run `34308440409`: external API-37 foreign-principal abuse probe installed a separate unprivileged attacker APK (`uid=10232`, ReAppzuku `uid=10231`), verified Accessibility/Quick Tile/Shizuku-provider/BOOT_COMPLETED entrypoints were denied, and proved no rejected probe bootstrapped `ShappkyService`;
- workflow run `34316502181`: Android 17/API 37 started official Shizuku, granted ReAppzuku through the real manager dialog, reached `SHIZUKU_READY`, executed representative typed `PrivilegedShell` mutation families through the real UserService and verified disposable-target rollback;
...
- workflow run `34153570390`: real API-36 official-Shizuku readiness and `ProcessRecord` parsing passed; its rejected synthetic background-service experiment is retained as historical evidence and is superseded for `ServiceRecord` runtime coverage by API-37 `35646519446`;
...
**Maintainability status:** the named high-risk seams now have explicit `PrivilegedShell`, `PackageStateSource`, `AppLaunchTriggerPolicy`, parser/protection/background policy, `Clock`/`ScheduleTime`, `AlarmScheduler`, `BackupCodec` and manual-restriction backup policies; the former SQL-debug redaction boundary has been removed entirely from production together with the obsolete debug subsystem. Machine parser casing is deterministic with `Locale.ROOT`. API-36 runtime covers the broad Shizuku/reboot/recovery baseline. Android 17/API 37 additionally proves real Shizuku privileged execution and now the production parser against a real system-generated `ServiceRecord` in `35646519446`. Root execution, physical/OEM parser/provider diversity and final stable-release signing/rollback evidence remain open.

# High-priority findings

## Closure status — 2026-09-21
